...
subroutine read_phantom_bin_files(iunit,n_files,filenames,x,y,z,h,vx,vy,vz,particle_id,massgas,massdust,&
 ...
 real(dp) :: hfact,umass,utime,ulength,gmw,x2,rhogasi
 ...
 elseif(got_u) then
    if (lemission_atom .and. ieos==12) then
       ...
       write(*,*) 'Calculating temperature from u, assuming ieos=',ieos,', gmw=',gmw
          do i=1,np
             hi = xyzh(4,i)
             rhogasi = massoftype(1,1)*(hfact/hi)**3  * umass/ulength**3 ! g/cm3, hard-coded ifile=1
 ...
 write(*,*) ' Maximum temperature is ',maxval(gastemperature),' Minimum temperature is ',minval(gastemperature)
